...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-temperature', type=int, default=0)
    ...
    temp_list = [30, 40, 50, 60, 70, 80]
    best_idx = temp_list.index(int(args.data_dir.split('sr')[-1]))
    with open(f'run/temperature/best_temp/ep50_round24_nnsat-{args.rnn_type}_1001_g1.txt', 'r') as f:
        temp = f.readline().strip().split(' ')
        best_temp = float(temp[best_idx])
        # print(best_temp)
    if args.test_temperature == 1:
        net.temperature = nn.Parameter(torch.Tensor(np.array([best_temp])).cuda())
    else:
        net.temperature = nn.Parameter(torch.Tensor(np.array([1])).cuda())
    ...
    if args.test_temperature == 1:
        with open(f'{args.output_dir}/temperature_iter{args.n_rounds}.txt', 'a+') as f:
